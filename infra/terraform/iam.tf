...
resource "aws_iam_role" "entity_mining_state_machine_role" {
  name = "entity-mining-state-machine-role"
...
resource "aws_iam_policy" "entity_mining_state_machine_policy" {
  name        = "entity-mining-state-machine-policy"
  description = "Allows Step Functions to invoke Lambda and update DynamoDB"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          aws_dynamodb_table.job_records.arn
        ]
      }
    ]
  })
...
resource "aws_iam_role_policy_attachment" "entity_mining_state_machine_policy" {
  role       = aws_iam_role.entity_mining_state_machine_role.name
  policy_arn = aws_iam_policy.entity_mining_state_machine_policy.arn
}